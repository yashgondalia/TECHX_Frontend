...
const Signup = () => {
  ...
  const handleLabChange = (lab: string) => {
    setFormData(prev => ({ ...prev, selectedLab: lab }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.email || !formData.username || !formData.fullName || 
        !formData.collegeName || !formData.mobileNumber) {
      ...
      <AuthCard>
        <div className="flex-1 pr-0 lg:pr-10">
          <div className="text-left mb-8">
            <h2 className="text-4xl font-bold mb-2 neon-glow-text">Sign Up</h2>
            <p className="text-gray-300 text-sm">Sign Up now to Enter the Event</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-1">
            ...
            <div className="text-center mt-4 text-gray-300">
              Already Registered? <Link to="/login" className="text-neon-cyan hover:underline">Login</Link>
            </div>
          </form>
        </div>
